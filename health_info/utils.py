...
def validate_health_data(data):
    ...
    try:
        ...
        if systolic < 50 or systolic > 250:
            raise ValidationError("Систолическое давление должно быть между 50 и 250")
        
        if diastolic < 30 or diastolic > 150:
            raise ValidationError("Диастолическое давление должно быть между 30 и 150")
        
        if systolic <= diastolic:
            raise ValidationError("Систолическое давление должно быть больше диастолического")
            
    except (ValueError, TypeError):
        raise ValidationError("Давление должно быть целым числом")
...
def import_from_xml(file_path):
    ...
    try:
        ...
        numeric_fields = [
            'age', 'height', 'weight', 'blood_pressure_systolic',
...
def save_health_data_from_dict(data):
    ...
    # Проверяем, существует ли уже пациент с таким ID
    if HealthData.objects.filter(patient_id=data['patient_id']).exists():
        raise ValidationError(f"Пациент с ID {data['patient_id']} уже существует")
